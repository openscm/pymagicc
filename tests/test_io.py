...
def generic_mdata_tests(mdata):
    ...
    assert mdata.data["scenario"].dtype == "object"
    assert mdata.data["model"].dtype == "object"
    assert mdata.data["climate_model"].dtype == "object"
...
def test_load_magicc6_conc():
    ...
    assert (mdata.data["unit"] == "ppm").all()
    generic_mdata_tests(mdata)
    assert_mdata_value(
        mdata,
        2.80435733e002,
...
def test_load_magicc7_emis_umlaut_metadata():
    ...
    assert (mdata.data["unit"] == "Gt C / yr").all()
...
def test_load_ot():
    ...
    assert (mdata.data["unit"] == "dimensionless").all()
    assert (mdata.data["todo"] == "SET").all()
    assert (
        mdata.data["variable"] == "Optical Thickness|NOx|MAGICC Fossil and Industrial"
    ).all()
...
def test_load_rf():
    ...
    assert (mdata.data["unit"] == "W / m^2").all()
    assert (mdata.data["todo"] == "SET").all()
    assert (mdata.data["variable"] == "Radiative Forcing|BC|MAGICC AFOLU").all()
...
def test_load_solar_rf():
    ...
    assert (mdata.data["unit"] == "W / m^2").all()
    assert (mdata.data["todo"] == "SET").all()
    assert (mdata.data["variable"] == "Radiative Forcing|Solar").all()
    assert (mdata.data["region"] == "World").all()
...
def test_load_volcanic_rf():
    ...
    assert (mdata.data["unit"] == "W / m^2").all()
    assert (mdata.data["todo"] == "SET").all()
    assert (mdata.data["variable"] == "Radiative Forcing|Volcanic").all()
...
def test_load_prn():
    ...
    assert (mdata.data["region"] == "World").all()
    assert (mdata.data["todo"] == "SET").all()
    assert not (mdata.data["unit"] == "t / yr").all()
...
def test_load_rcp_historical_dat_emissions():
    ...
    assert (mdata.data["variable"].str.startswith("Emissions|")).all()
    assert (mdata.data["region"] == "World").all()
    assert (mdata.data["todo"] == "SET").all()
...
def test_load_rcp_historical_dat_concentrations():
    ...
    assert (mdata.data["variable"].str.startswith("Atmospheric Concentrations|")).all()
    assert (mdata.data["region"] == "World").all()
    assert (mdata.data["todo"] == "SET").all()
...
def test_load_rcp_historical_dat_forcings():
    ...
    assert (mdata.data["variable"].str.startswith("Radiative Forcing")).all()
    assert (mdata.data["region"] == "World").all()
    assert (mdata.data["todo"] == "SET").all()
    assert (mdata.data["unit"] == "W / m^2").all()
...
def test_load_out():
    ...
    assert (mdata.data.todo == "N/A").all()
    assert (mdata.data.unit == "K").all()
    assert (mdata.data.variable == "Surface Temperature").all()
...
def test_load_out_slash_and_caret_in_rf_units():
    ...
    assert (mdata.data["todo"] == "N/A").all()
    assert (mdata.data["unit"] == "W / m^2").all()
    assert (mdata.data["variable"] == "Radiative Forcing|SOx|MAGICC AFOLU").all()
...
def test_bin_and_ascii_equal(file_to_read):
    ...
    assert (mdata_bin.data["unit"] == "unknown").all()
    assert (mdata_bin.data["todo"] == "SET").all()
    ...
    drop_axes = ["unit", "todo"]
    mdata_ascii.data = mdata_ascii.data.drop(drop_axes, axis="columns")
    mdata_bin.data = mdata_bin.data.drop(drop_axes, axis="columns")
    pd.testing.assert_frame_equal(mdata_ascii.data, mdata_bin.data)
