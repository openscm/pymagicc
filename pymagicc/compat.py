...
def _determine_version(magicc_dir):
    ...
    if ('FILE_EMISSIONSCENARIO' in user_cfg['nml_allcfgs'] and
       'FILE_EMISSCEN_2' in user_cfg['nml_allcfgs']):
        raise ValueError(
            'Invalid MAGCFG_USER.CFG. Should not contain both '
            'FILE_EMISSIONSCENARIO and FILE_EMISSCEN_2 keys'
            )
...
def determine_version():
    ...
    Uses expected parameters in the MAGCFG_USER.CFG configuration file.
    The result is cached as it can be expensive to calculate.
    """
...
def get_param(key):
    ...
    try:
        ...
        raise ValueError(
            'Invalid key ({}) for version {}'.format(key, version))
